...
EngineInfo parseEngineInfo({required String input, required String fen}) {
  ...
  final pvMatch = RegExp(r'(?<!multi)pv (.*?)(?=\snodes|\stime|\shashfull|$)').firstMatch(input);
  ...
  String sans = '';
  try {
    sans = getSanMovesFromfenAndNotations(fen: fen, chainNotation: moves);
  } catch (e) {
    print(e);
  }
  ...
  // Tính toán điểm số
  final calculatedScore = score>0?calcEval(score).round(): (- calcEval(score.abs()).round());

  // Tạo title
  final title = 'Độ sâu $depth, điểm số $calculatedScore, nps ${npsK}k';
